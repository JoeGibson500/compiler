...
char currentMethodReturnType[128];
...
ParserInfo classVarDeclar() {
	...
	strcpy(currentMethodReturnType, t.lx);
	...
	if(t.tp == ID) {
		...
		if (parserPhase == 1) {
			if (FindMethodSymbol(currentClassName, currentVariableName)) {
				...
				InsertMethodSymbol(currentClassName, currentVariableName, currentMethodReturnType);
			}
		}
	} else {
		//// printf("Identifier not found\n");
		...
		if (t.tp == ID) {
			...
			if (parserPhase == 1) {
				if (FindMethodSymbol(currentClassName, currentMethodName)) {
					...
					InsertMethodSymbol(currentClassName, currentMethodName, currentMethodReturnType);
				}
			}
...
ParserInfo type() {
	...
	if (!strcmp(t.lx, "int") || !strcmp(t.lx, "char") || !strcmp(t.lx, "boolean")) {
		...
		if (parserPhase == 2) {
			...
			if (FindClassSymbol(currentVariableName) || IsJackLibraryClass(currentVariableName)) {		
...
ParserInfo paramList() { 
	...
	if (isLexerError()) return pi;
		...
		if (t.tp == ID) {
			...
			strcpy(currentVariableName, t.lx);
			if (parserPhase == 1) {
				InsertVariableSymbol(currentClassName, currentMethodName, currentVariableName, currentType);
...
ParserInfo subroutineCall(){
	...
	if (t.tp == ID) {
		...
		//// printf("Class instance name :%s\n", classInstanceName);
		// strcpy(currentVariableName, t.lx);
		if (parserPhase == 2) {

			if (FindClassSymbol(classInstanceName)) {
				// Do nothing
			} else if (FindVariableSymbol(currentClassName, currentMethodName, classInstanceName)) {

				// // printf("FOUND");
				strcpy(currentType, FindVariableType(currentClassName, currentMethodName, classInstanceName));
				
				// printf("current type = %s\n", currentType);
			} else if (FindMethodSymbol(currentClassName, classInstanceName)){

				strcpy(currentMethodReturnType, FindMethodReturnType(currentClassName, classInstanceName));

			} else if (IsJackLibraryClass(classInstanceName)) {

				strcpy(currentJackLibraryClass, t.lx); 
				//// printf("CURRENT JACK LIBRARY CLASS =%s\n", currentJackLibraryClass);
		...
		if (t.tp == ID) {
			...
			if (parserPhase == 2) {
				 if (FindMethodSymbol(classInstanceName, classInstanceMethodName)) {
					// Do nothing
				} else if (FindMethodSymbol(currentType, classInstanceMethodName)) {
					// Do nothing
				} else if (FindMethodSymbol(currentMethodReturnType, classInstanceMethodName)) {
					// Do nothing
				} else if (IsJackLibraryMethod(currentJackLibraryClass, classInstanceMethodName)) {
					// Do nothing also 
				} else if (IsJackLibraryMethod(currentType, classInstanceMethodName)) {
					// Do nothing also 
				} else {
					//// printf("Undeclared identifier");
					pi.er = undecIdentifier;
...
ParserInfo operand() {
	...
	if (isIntegerConstant(t.lx)) { // Integer constant
		...
		if (parserPhase == 2) {
			...
			if (FindClassSymbol(currentVariableName)) {
				// Do nothing

			} else if (FindMethodSymbol(currentClassName, currentVariableName)) {
				// Do nothing
				strcpy(currentMethodReturnType, FindMethodReturnType(currentClassName, currentVariableName));


			} else if (FindVariableSymbol(currentClassName, currentMethodName, currentVariableName)) {
				// Do nothing 
				strcpy(currentType, FindVariableType(currentClassName, currentMethodName, currentVariableName));
				strcpy(currentJackLibraryClass, FindVariableType(currentClassName, currentMethodName, currentVariableName));

			} else if (IsJackLibraryClass(currentVariableName)) {
				// Do nothing
				strcpy(currentJackLibraryClass, t.lx); 
...
int InitParser (char* file_name) {
	...
	char lastChar = file_name[len-1];

	// Convert last character to int
	parserPhase = lastChar - '0';

	char new_file_name[len];
	strncpy(new_file_name, file_name, len-1);
	new_file_name[len-1] = '\0'; // Null terminate 

	// //// printf("Opening file %s\n", new_file_name);
	InitLexer(new_file_name);
	return 1;
